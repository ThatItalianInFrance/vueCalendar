...
console.log(employee);
  ...
  const employeePlannings = plannings.filter(
    (planning) => planning.employee_id === employeeId
  );
  ...
  employeePlannings.forEach((planning) => {
    ...
    while (currentDate <= end) {
      ...
      if (holidays.some((holiday) => new Date(holiday.holiday_date).toDateString() === currentDate.toDateString())) {
      ...
      if (days_of_the_week.includes(dayOfWeek)) {
        ...
        if (
          (weeks_of_the_month.length > 0 && !weeks_of_the_month.includes(weekNumber(currentDate))) ||
...
export async function generateSingleEmployeeScheduleWithRecurrences(
  ...
  employeePlannings.forEach((planning) => {
    ...
    while (currentDate <= end) {
      ...
      if (months_of_the_year && months_of_the_year.length > 0 && !months_of_the_year.includes(monthNumber)) {
        currentDate.setDate(currentDate.getDate() + 1);
        continue;
      }
      ...
      if (weeks_of_the_month && weeks_of_the_month.length > 0 && !weeks_of_the_month.includes(weekNumber)) {
        currentDate.setDate(currentDate.getDate() + 1);
        continue;
      }
